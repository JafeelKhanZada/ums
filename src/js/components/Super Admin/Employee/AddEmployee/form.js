...
function Forms() {
  ...
  const value = useSelector(({ Employee }) => Employee);
  const hasError = (inputName) => !!(errors && errors[inputName]);
  const [batch, setBatch] = useState([]);
  useEffect(() => {
    setBatch(value.PROGRAM);
  }, [value]);
  const onSubmit = (values) => {
    ...
    dispatch(Action.postEmployeeInfo(obj)).then((res) => {
      if (res) {
        reset();
      }
    });
                ...
                <Controller
                  ...
                  as={
                    <Select
                      ...
                      className={classes.InputContainer}
                    >
                      <MenuItem disbled>Select Program</MenuItem>;
                      {batch?.map((v, k) => {
            ...
            <Grid item xs={6}>
              ...
              >
                ...
                <Controller
                  rules={{ required: true }}
                  control={control}
                  defaultValue={getValues("joiningDate")}
                  name="joiningDate"
                  as={
                    <MuiPickersUtilsProvider utils={DateFnsUtils}>
                      <KeyboardDatePicker
                ...
                <FormHelperText>
                  {hasError("joiningDate") && `Date Of Birth is mandatory`}
                </FormHelperText>
              </FormControl>
            </Grid>
            <Grid item xs={6}>
